        ...
        <div className="config-section">
            ...
            <div className="config-card" style={{ background: '#f5f3ff', border: '1px solid #ddd6fe' }}>
                ...
                <div className="form-group">
                    <div style={{ display: 'flex', gap: '10px' }}>
                        <input type="text" readOnly value={storeUrl ? `${storeUrl}/pages/app` : "Carregando..."} style={{ backgroundColor: 'white', color: '#555', flex: 1, padding: '10px', borderRadius: '6px', border: '1px solid #ccc' }} />
                        <button onClick={copyLink} style={{ background: '#8B5CF6', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', padding: '0 20px', fontWeight: 'bold' }}>Copiar</button>
                    </div>
                </div>
            </div>
            ...
            <div className="config-card" style={{ flexDirection: 'row', alignItems: 'center', gap: '20px', display: 'flex' }}>
                ...
                <div>
                    ...
                    <a href={qrCodeUrl} download="qrcode.png" target="_blank" rel="noreferrer" style={{color: config.theme_color, textDecoration: 'none', fontWeight: 'bold', fontSize: '14px'}}>⬇️ Baixar Imagem</a>
                </div>
            </div>
            ...
            <div className="config-card">
                ...
                <div className="form-group">
                    ...
                    <input type="text" value={config.app_name} onChange={(e) => setConfig({...config, app_name: e.target.value})} placeholder="Ex: Minha Loja Oficial" />
                </div>
                
                <div className="form-group">
                    <label>Cor Principal (Tema)</label>
                    <div className="color-picker-wrapper">
                        <input type="color" value={config.theme_color} onChange={(e) => setConfig({...config, theme_color: e.target.value})} />
                        <input type="text" value={config.theme_color} onChange={(e) => setConfig({...config, theme_color: e.target.value})} className="color-text" />
                    </div>
                </div>
            ...
            {/* Widgets de Conversão (ATUALIZADO) */}
            <div className="config-card">
                ...
                {/* Opções Expandidas do Widget */}
                {config.fab_enabled && (
                    <div className="animate-fade-in" style={{background: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '15px'}}>
                        
                        <div style={{ display: 'flex', gap: '15px', marginBottom: '15px' }}>
                            <div style={{ flex: 1 }}>
                                <label style={{fontSize:'12px', fontWeight:'bold', color:'#374151', display:'block', marginBottom:'5px'}}>Texto do Botão</label>
                                <input type="text" value={config.fab_text || "Baixar App"} onChange={(e) => setConfig({...config, fab_text: e.target.value})} placeholder="Ex: Instalar Agora" style={{width:'100%', padding:'8px', border:'1px solid #d1d5db', borderRadius:'6px'}} />
                            </div>
                            <div style={{ width: '80px' }}>
                                <label style={{fontSize:'12px', fontWeight:'bold', color:'#374151', display:'block', marginBottom:'5px'}}>Ícone</label>
                                <input type="text" value={config.fab_icon || "📲"} onChange={(e) => setConfig({...config, fab_icon: e.target.value})} style={{width:'100%', padding:'8px', border:'1px solid #d1d5db', borderRadius:'6px', textAlign:'center'}} />
                            </div>
                        </div>

                        <div style={{ display: 'flex', gap: '15px' }}>
                            <div style={{ flex: 1 }}>
                                <label style={{fontSize:'12px', fontWeight:'bold', color:'#374151', display:'block', marginBottom:'5px'}}>Posição na Tela</label>
                                <select 
                            ...
                            <div style={{ flex: 1 }}>
                                <label style={{fontSize:'12px', fontWeight:'bold', color:'#374151', display:'block', marginBottom:'5px'}}>
                                ...
                                />
                                <small style={{fontSize:'10px', color:'#666'}}>Tempo para aparecer após abrir o site.</small>
                            </div>
                        </div>
                    </div>
                )}
        ...
        <div className="preview-section">
            <div className="sticky-wrapper">
                ...
                <PhonePreview 
                    ...
                    // Passando novas props para o preview (mesmo que ele não use todas ainda, não quebra)
