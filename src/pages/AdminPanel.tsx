...
export default function AdminPanel() {
  ...
  // carrega config + stats + info da loja
  useEffect(() => {
    if (!token) return;
    setLoading(true);
    ...
    Promise.all([
      ...
      .then(([dataConfig, dataStats, dataUrl]) => {
        ...
        setStats({
          receita: dataStats.receita ?? 0,
          vendas: dataStats.vendas ?? 0,
          instalacoes: dataStats.instalacoes ?? 0,
          carrinhos_abandonados:
            dataStats.carrinhos_abandonados ?? { valor: 0, qtd: 0 },
          taxa_conversao: dataStats.taxa_conversao ?? { app: 0, site: 0 },
          economia_ads: dataStats.economia_ads ?? 0,
          top_produtos: dataStats.top_produtos ?? [],
          visualizacoes:
            dataStats.visualizacoes ?? {
              pageviews: 0,
              tempo_medio: "--",
              top_paginas: [],
            },
          funil:
            dataStats.funil ?? { visitas: 0, carrinho: 0, checkout: 0 },
          recorrencia:
            dataStats.recorrencia ?? { clientes_2x: 0, taxa_recompra: 0 },
          ticket_medio:
            dataStats.ticket_medio ?? { app: 0, site: 0 },
          visitas: dataStats.visitas, // opcional, se o back mandar
        });
