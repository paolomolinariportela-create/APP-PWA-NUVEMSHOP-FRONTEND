        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }} onClick={onClose}>
            <div style={{ background: '#fff', borderRadius: '16px', width: '100%', maxWidth: '720px', maxHeight: '92vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }} onClick={e => e.stopPropagation()}>
                ...
                <div style={{ padding: '20px 24px' }}>
                    <div style={{ background: '#EEF2FF', border: '1px solid #C7D2FE', borderRadius: '10px', padding: '12px 16px', marginBottom: '20px', fontSize: '12px', color: '#3730A3', lineHeight: 1.6 }}>
                            ...
                            return (
                                <div key={v} style={{ border: `2px solid ${cor}`, borderRadius: '12px', padding: '16px', background: corBg }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px' }}>
                                        ...
                                        <span style={{ marginLeft: 'auto', fontSize: '11px', color: cor, background: isA ? '#dbeafe' : '#dcfce7', padding: '2px 8px', borderRadius: '999px' }}>≈{metade.toLocaleString('pt-BR')}</span>
                                    </div>
                                    <div style={{ marginBottom: '10px' }}>
                                        ...
                                        <small style={{ fontSize: '10px', color: '#6B7280' }}>{form[titleKey].length}/50</small>
                                    </div>
                                    <div>
                                        ...
                                        <small style={{ fontSize: '10px', color: '#6B7280' }}>{form[msgKey].length}/120</small>
                                    </div>
                                    {(form[titleKey] || form[msgKey]) && (
                                        <div style={{ marginTop: '10px', background: '#111827', borderRadius: '8px', padding: '8px 12px', display: 'flex', gap: '8px' }}>
                                            ...
                                            <div>
                                                <div style={{ fontSize: '11px', fontWeight: 700, color: '#fff' }}>{form[titleKey] || `Título ${v}`}</div>
                                                <div style={{ fontSize: '10px', color: '#9CA3AF', marginTop: '1px' }}>{form[msgKey] || `Mensagem ${v}`}</div>
                                            </div>
                                        </div>
                                    )}
                                </div>
                    ...
                    <div style={{ background: '#F0FDF4', border: '1px solid #86efac', borderRadius: '10px', padding: '14px 16px', marginBottom: '16px' }}>
                        ...
                        {form.auto_escalar && (
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                ...
                                <select value={form.horas_avaliacao ?? 4} onChange={e => setForm({ ...form, horas_avaliacao: parseInt(e.target.value) })} style={{ padding: '6px 10px', border: '1px solid #86efac', borderRadius: '6px', background: 'white', fontSize: '12px', color: '#166534' }}>
                                ...
                                <span style={{ fontSize: '11px', color: '#15803D' }}>— vencedor enviado para toda a base</span>
                            </div>
                        )}
                    </div>
