        ...
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                <div>
                    ...
                    <div style={{ fontSize: '13px', color: C.textSoft, marginTop: '2px' }}>{data.total_visitors.toLocaleString('pt-BR')} visitors classificados por comportamento</div>
                </div>
                <button onClick={fetchScores} style={{ background: 'none', border: `1px solid ${C.neutralBorder}`, borderRadius: '6px', padding: '5px 10px', cursor: 'pointer', fontSize: '13px', color: C.textMid, display: 'inline-flex', alignItems: 'center', gap: '4px' }}>{Icon.refresh} Atualizar</button>
            </div>
                            ...
                            <div onClick={() => g.count > 0 && setGrupoAberto(aberto ? null : g.id)} style={{ padding: '12px 14px', background: aberto ? g.corBg : C.white, cursor: g.count > 0 ? 'pointer' : 'default', display: 'flex', alignItems: 'center', gap: '10px', transition: 'background 0.2s' }}>
                                <div style={{ flex: 1 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '2px' }}>
                                        ...
                                        <span style={{ background: g.corBg, color: g.cor, border: `1px solid ${g.cor}30`, padding: '1px 8px', borderRadius: '999px', fontSize: '13px', fontWeight: 600 }}>{g.count.toLocaleString('pt-BR')}</span>
                                        <span style={{ fontSize: '13px', color: C.neutralLight }}>{g.pct}%</span>
                                    </div>
                                    <div style={{ fontSize: '13px', color: C.textSoft }}>{g.desc}</div>
                                </div>
                                ...
                                {g.count > 0 && (
                                    <button onClick={e => { e.stopPropagation(); onSegmentar(g.segmento_os, g.label); }} style={{ padding: '5px 10px', borderRadius: '6px', border: `1px solid ${g.cor}`, background: C.white, color: g.cor, fontSize: '13px', fontWeight: 600, cursor: 'pointer', flexShrink: 0 }} onMouseOver={e => { e.currentTarget.style.background = g.corBg; }} onMouseOut={e => { e.currentTarget.style.background = C.white; }}>
                                        Enviar push
                                    </button>
                                )}
                                {g.count > 0 && <span style={{ color: C.neutralLight, fontSize: '13px', flexShrink: 0 }}>{aberto ? '▲' : '▼'}</span>}
                            </div>
                                ...
                                <div style={{ borderTop: `1px solid ${g.cor}20`, background: g.corBg, padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                    {g.visitors.map((v, i) => (
                                        <div key={i} style={{ background: C.white, borderRadius: '6px', padding: '7px 10px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                            ...
                                            <div style={{ flex: 1, fontFamily: 'monospace', fontSize: '13px', color: C.textMid }}>{v.visitor_id.substring(0, 20)}…</div>
                                            <div style={{ fontSize: '13px', color: C.textSoft }}>{formatRelative(v.ultima_visita)}</div>
                                    ...
                                    {g.count > g.visitors.length && <div style={{ textAlign: 'center', fontSize: '13px', color: C.neutralLight, paddingTop: '2px' }}>+ {g.count - g.visitors.length} não exibidos</div>}
                                </div>
                            )}
                        </div>
                    );
            ...
            <div style={{ marginTop: '14px', padding: '10px 14px', background: C.neutralBg, borderRadius: '8px', fontSize: '13px', color: C.textSoft, lineHeight: 1.6 }}>
