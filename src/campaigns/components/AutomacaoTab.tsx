            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: ativo ? '14px' : '0' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: cor, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <span style={{ color: '#fff', fontSize: '12px', fontWeight: 700 }}>{passo}</span>
                    </div>
                    <div>
                        <div style={{ fontWeight: 600, fontSize: '13px', color: C.text }}>{label}</div>
                        <div style={{ fontSize: '11px', color: C.textSoft }}>{ativo ? `Envia após ${horas >= 1 ? `${horas}h` : '30 min'}` : 'Desativado'}</div>
                    </div>
                </div>
            ...
            {ativo && (
                <div>
                    <div style={{ marginBottom: '10px' }}>
                        <label style={{ fontSize: '12px', fontWeight: 600, color: C.textMid, display: 'block', marginBottom: '4px' }}>Enviar após</label>
                    ...
                    <div style={{ marginBottom: '10px' }}>
                        <label style={{ fontSize: '12px', fontWeight: 600, color: C.textMid, display: 'block', marginBottom: '4px' }}>Título</label>
                        ...
                        <small style={{ fontSize: '10px', color: C.neutralLight }}>{titulo.length}/50</small>
                    </div>
                    <div style={{ marginBottom: passo === 3 ? '10px' : '0' }}>
                        <label style={{ fontSize: '12px', fontWeight: 600, color: C.textMid, display: 'block', marginBottom: '4px' }}>Mensagem</label>
                        ...
                        <small style={{ fontSize: '10px', color: C.neutralLight }}>{mensagem.length}/120</small>
                    </div>
                    {passo === 3 && (
                        <div style={{ marginBottom: '0' }}>
                            <label style={{ fontSize: '12px', fontWeight: 600, color: C.textMid, display: 'block', marginBottom: '4px' }}>Cupom (opcional)</label>
                    ...
                    <div style={{ marginTop: '10px', background: C.dark, borderRadius: '8px', padding: '8px 12px', display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                        ...
                        <div>
                            <div style={{ fontSize: '11px', fontWeight: 600, color: '#fff' }}>{titulo || 'Título'}</div>
                            <div style={{ fontSize: '10px', color: C.neutralLight, marginTop: '1px' }}>{mensagem || 'Mensagem'}{passo === 3 && cupom ? ` · Cupom: ${cupom}` : ''}</div>
                        </div>
                    </div>
                </div>
            )}
    ...
    return (
        <div className="config-card">
            ...
            <div style={{ background: C.brandLight, border: `1px solid ${C.brandMuted}30`, borderRadius: '8px', padding: '11px 13px', marginBottom: '20px', display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                ...
                <div style={{ fontSize: '12px', color: C.brand, lineHeight: 1.5 }}><strong>Como funciona:</strong> Quando um cliente abandona o carrinho, o sistema aguarda o tempo configurado e envia a notificação automaticamente.</div>
            </div>
            ...
            <div style={{ borderTop: `1px solid ${C.neutralBorder}`, paddingTop: '16px', marginTop: '4px', marginBottom: '12px' }}>
                <div style={{ fontSize: '13px', fontWeight: 600, color: C.textMid, display: 'flex', alignItems: 'center', gap: '6px' }}>
                    {Icon.eye} Produto Visitado <span style={{ fontSize: '11px', fontWeight: 400, color: C.neutralLight }}>— lembrete automático após visitar produto</span>
                </div>
            </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: automacao.produto_visitado_ativo ? '14px' : '0' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <div>
                            ...
                            <div style={{ fontSize: '11px', color: C.textSoft }}>{automacao.produto_visitado_ativo ? `Lembrete após ${automacao.produto_visitado_horas}h` : 'Desativado'}</div>
                        </div>
                    </div>
            ...
            <div style={{ borderTop: `1px solid ${C.neutralBorder}`, paddingTop: '16px', marginTop: '4px', marginBottom: '12px' }}>
                <div style={{ fontSize: '13px', fontWeight: 600, color: C.textMid, display: 'flex', alignItems: 'center', gap: '6px' }}>
                    {Icon.clock} Cliente Inativo <span style={{ fontSize: '11px', fontWeight: 400, color: C.neutralLight }}>— reativação automática de clientes sumidos</span>
                </div>
            </div>
            <div style={{ border: `2px solid ${automacao.inativo_ativo ? '#f59e0b' : C.neutralBorder}`, borderRadius: '10px', padding: '16px', marginBottom: '12px', background: automacao.inativo_ativo ? '#fafafa' : C.neutralBg, transition: 'all 0.2s' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: automacao.inativo_ativo ? '14px' : '0' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <div>
                            ...
                            <div style={{ fontSize: '11px', color: C.textSoft }}>{automacao.inativo_ativo ? `Dispara após ${automacao.inativo_dias} dias sem visita` : 'Desativado'}</div>
                        </div>
                    </div>
                ...
                {automacao.inativo_ativo && (
                    <div>
                        <div style={{ marginBottom: '10px' }}>
                            <label style={{ fontSize: '12px', fontWeight: 600, color: C.textMid, display: 'block', marginBottom: '4px' }}>Dias de inatividade</label>
                            ...
                            <small style={{ fontSize: '10px', color: C.neutralLight }}>Baseado na tag ultima_visita — roda 1x por dia</small>
                        </div>
                        <div style={{ marginBottom: '10px' }}>
                            <label style={{ fontSize: '12px', fontWeight: 600, color: C.textMid, display: 'block', marginBottom: '4px' }}>Título</label>
