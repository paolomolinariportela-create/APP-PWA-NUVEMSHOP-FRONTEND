...
const TRIGGER_META: Record<TriggerType, { label: string; icon: string; description: string }> = {
    ...
    <div style={S.triggerCard}>
      ...
      {config.type === 'time_on_page' && (
        <div>
          ...
          <input type="number" min={1} max={300} value={config.seconds}
      ...
      {config.type === 'scroll_depth' && (
        <div>
          ...
          <input type="range" min={5} max={100} step={5} value={config.percent}
      ...
      {config.type === 'exit_intent' && (
        <div>
          ...
          <select value={config.sensitivity ?? 'medium'}
            onChange={(e) => onChange({ ...config, sensitivity: e.target.value as 'low' | 'medium' | 'high' })}
      ...
      {config.type === 'inactivity' && (
        <div>
          ...
          <input type="number" min={5} max={600} value={config.seconds}
            onChange={(e) => onChange({ ...config, seconds: Number(e.target.value) })}
            style={S.input} />
        </div>
      )}
      {config.type === 'on_load' && (
        <div>
          ...
          <input type="number" min={0} max={10000} step={100} value={config.delayMs ?? 0}
  ...
  return (
    <div style={S.root}>
      ...
      <div style={{ marginBottom: '16px' }}>
        <input placeholder="Ex: Popup após 10s ou 50% scroll" value={rule.label ?? ''}
          onChange={(e) => update({ label: e.target.value })} style={S.input} />
      </div>
        ...
        <div style={{ marginBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {(['OR', 'AND'] as TriggerOperator[]).map((op) => (
              <button key={op} style={{ ...S.badge(rule.operator === op), fontSize: '12px', padding: '6px 16px' }}
      ...
      {rule.triggers.length === 0 && (
        <div style={{ textAlign: 'center', padding: '20px', color: '#334155', fontSize: '12px',
          background: '#0F172A', borderRadius: '10px', border: '1px dashed #1E293B', marginBottom: '12px' }}>
          Nenhum gatilho configurado.<br />Adicione pelo menos um abaixo.
        </div>
      )}

      {rule.triggers.map((cfg, idx) => (
        <TriggerItem key={idx} config={cfg}
        ...
        <div style={{ ...S.triggerCard, marginBottom: '10px' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '6px', marginBottom: '10px' }}>
            {availableTypes.map((t) => (
              <button key={t} style={S.badge(addingType === t)} onClick={() => setAddingType(t)}>
                {TRIGGER_META[t].icon} {TRIGGER_META[t].label}
              </button>
            ))}
          </div>
          <div style={{ display: 'flex', gap: '8px' }}>
            <button style={{ ...S.saveBtn, marginTop: 0, flex: 1 }} onClick={() => addTrigger(addingType)}>
              Confirmar
            </button>
            <button style={{ flex: 1, background: '#1E293B', border: '1px solid #334155', borderRadius: '10px',
              padding: '12px', color: '#64748B', fontWeight: 600, fontSize: '13px', cursor: 'pointer' }}
      ...
      <div style={S.row}>
        ...
        <input type="number" min={0} max={10} value={rule.maxFiresPerSession ?? 1}
          onChange={(e) => update({ maxFiresPerSession: Number(e.target.value) })}
          style={{ ...S.input, width: '70px', textAlign: 'center' }} />
      </div>

      <button style={S.saveBtn} onClick={() => onChange(rule)} disabled={rule.triggers.length === 0}>
        Salvar gatilhos
      </button>
    </div>
  );
